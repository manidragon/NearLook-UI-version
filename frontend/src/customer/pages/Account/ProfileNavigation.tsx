...
const menu = [
  ...
  { name: "Following", path: "/account/following", icon: <StorefrontIcon sx={{ fontSize: 22 }} /> },
  ...
  return (
    <div className="w-full md:w-[28%] lg:w-[20%] flex-shrink-0 space-y-4 sticky top-4">
      ...
      <div className="bg-gradient-to-r from-[#FF5A00] to-[#ff8447] shadow-md rounded-2xl p-5 flex items-center gap-4 relative overflow-hidden">
        ...
        <img 
          src={user.user?.profilePicture || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.user?.fullName || 'User')}&background=FF5A00&color=fff`} 
          alt="Profile" 
          ...
          return (
            <div
              ...
              className={`
                ...
                ${isActive ? "text-[#c24100] bg-orange-50/50 font-bold" : "text-gray-600"}
                px-5 py-4 hover:bg-orange-50 hover:text-[#c24100] cursor-pointer transition-all duration-300 flex items-center justify-between z-10
              `}
            >
              ...
              <ChevronRightIcon sx={{ fontSize: 20, color: isActive ? "#c24100" : "#d1d5db" }} />
            </div>
          );
        })}
      </div>
