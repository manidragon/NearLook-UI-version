    ...
    <div className='p-5 md:p-6 border border-gray-100 rounded-2xl bg-white hover:shadow-md transition-shadow relative overflow-hidden'>
      <div className='space-y-3 text-[14px] text-gray-600'>
        <div className='flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 pb-2 border-b border-gray-50'>
          ...
          <span className='font-medium text-gray-700 bg-gray-50 px-3 py-1 rounded-full text-[12px] w-fit'>{item.mobile}</span>
        </div>
      ...
      <div className="mt-6 pt-4 border-t border-gray-50 flex gap-4 justify-end">
        <button 
          onClick={() => onEdit(item)}
          className="font-semibold text-gray-700 hover:text-[#c24100] text-[13px] tracking-wide transition-colors uppercase"
        >
          Edit
        </button>
        <button 
          onClick={() => onDelete(item._id)}
          className="font-semibold text-red-700 hover:text-red-800 text-[13px] tracking-wide transition-colors uppercase"
        >
          Delete
        </button>
      </div>
