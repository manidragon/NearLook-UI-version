...
const marker = "const renderSellerOffers = () => {";
              ...
              <div className="productContent w-full lg:w-[60%] md:pr-10 md:pl-10 lg:pl-14">
                ...
                <div className="flex items-start sm:items-center flex-col sm:flex-row md:flex-row lg:flex-row gap-3 justify-start">
                  ...
                  {(() => {
                    ...
                    const totalStars = review.reviews?.reduce((sum, item) => sum + (item.rating || 0), 0) || 0;
                    ...
                    <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                      {colorsWithImages.map((colorData, index) => {
                        const firstVariant = sellerIdFromProfile
                          ? colorData.variants.find((variant) =>
                            variant.offers?.some((o) => String(o.seller?._id || o.seller) === String(sellerIdFromProfile) && o.isActive !== false && (o.stock ?? 0) > 0)
                          )
                  ...
                  <div className="mt-6">
                    ...
                    <div className="flex gap-3 flex-wrap">
                      {availableVariantsForColor.map((variant) => {
                        ...
                        const hasActiveOffer = variant.offers?.some((o) => o.isActive !== false && (o.stock ?? 0) > 0);
                ...
                {isCatalogProduct && sellerOffers.length > 0 && selectedVariantId && (
                  <div className="my-8 text-sm space-y-3">
                    ...
                    {sellerOffers.map((offer) => {
                      const variant = offer.variants.find((v) => v._id === selectedVariantId);
                      ...
                      const sellerAvgRating = sellerReviews.length > 0 ? (sellerReviews.reduce((sum, r) => sum + Number(r.rating || 0), 0) / sellerReviews.length).toFixed(1) : null;
                ...
                {activeTab === 2 && (
                  <div className="max-w-4xl">
                    <div className="space-y-6">
                      {(review.reviews || []).slice().sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 5).map((item, index) => (
      ...
      <Drawer anchor="right" open={openSellerDrawer} onClose={() => setOpenSellerDrawer(false)}>
        <Box sx={{ width: 580, p: 3, bgcolor: '#f8f8f8', height: '100%' }}>
          ...
          <Box ref={reviewScrollRef} sx={{ display: 'flex', overflowX: 'auto', gap: 2, pb: 2, scrollBehavior: 'smooth', position: 'relative', '&::-webkit-scrollbar': { display: 'none' } }}>
            {[...selectedSellerReviews].reverse().map((rev, i) => (
...
const newContent = content.substring(0, returnIdx) + newJsx;
